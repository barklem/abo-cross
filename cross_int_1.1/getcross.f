corrections
c  
c  291298 DGAMMA corrected to be declared real*8 in WIDTH
c         PARAMETER statements changed to more standard format
c
c
	program getcross
c**********************************************************************
c This is a calling program to return an interpolated cross-section
c  for NEUTRALS
c Uses bicubic spline interpolation
c  You may not want to actually use this calling routine,
c  you may just want the subs.
...
c
	implicit none
	real*4 NSupp,NSlow,CROSS,ALPHA,HALFWIDTH,FULLWIDTH,TEMP,A
	real*4 LGFW 
	integer Lupp,Llow,IFAIL
c
c  get the input n*, and l (angular momentum quantum numbers)
c  s,p,d,f... --> 0,1,2,3,... etc
c
10	continue
c
	print *,'Enter n* for the lower state --> '
	read(*,*) NSlow
	print *,'Enter n* for the upper state --> '
	read(*,*) NSupp
	print *,'Enter l for the lower state --> '
	read(*,*) Llow
	print *,'Enter l for the upper state --> '
	read(*,*) Lupp
	print *,'Enter the absorber Atomic Mass --> '
	read(*,*) A
c You can get an estimate by just making A something big
	print *,'Enter Temperature --> '
	read(*,*) TEMP
c
	call RETCROSS(NSlow,NSupp,Llow,Lupp,CROSS,ALPHA,IFAIL)
	call WIDTH(CROSS,ALPHA,TEMP,A,HALFWIDTH)
	FULLWIDTH=2.*HALFWIDTH
c
74	format(7A10)
75      format(2F10.3,2I10,2F10.3,I10)
76      format(A30,E10.4)
77	format(A30,F10.4)
	print *,' '
	write(6,74)'NSlow','NSupp','Llow','Lupp','CROSS',
     ;             'ALPHA','IFAIL'
	write(6,75)NSlow,NSupp,Llow,Lupp,CROSS,ALPHA,IFAIL
	write(6,76)'w/N (cm^3 rad/s) =  ',HALFWIDTH
	write(6,76)'Gamma/N (cm^3 rad/s) =  ',FULLWIDTH
	LGFW = 0.
	if (IFAIL.eq.0) LGFW = LOG10(FULLWIDTH)
	write(6,77)'Log(Gamma/N) = ',LGFW
c
	end
